...
void tp_sap_udata_ind(enum tp_sap_data_type type, const uint8_t *bits, unsigned int len, struct tetra_mac_state *tms)
{
	...
	if ((type == TPSAP_T_SCH_F) && (tms->cur_burst.is_traffic || tms->channel_type == TETRA_TYPE_DIRECT) && tms->dumpdir) {
		...
		/* Open target file */
		snprintf(fname, sizeof(fname), "%s/traffic_%d_%d.out", tms->dumpdir, tms->cur_burst.is_traffic, tms->tsn);
		f = fopen(fname, "ab");
		...
		/* Write used ssi */
		snprintf(fname, sizeof(fname), "%s/traffic_%d_%d.txt", tms->dumpdir, tms->cur_burst.is_traffic, tms->tsn);
		f=fopen(fname, "a");
		fprintf(f, "%d\n", tms->ssi);
		fclose(f);
	}
